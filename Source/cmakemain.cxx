...
int do_reprobuild_launch(int ac, char const* const* av)
{
  ...
  if (!sourceDir.empty() && action == "build") {
    ...
    std::string const providerFile = cmStrCat(buildDir, "/reprobuild.nim");
    std::string const providerStateFile =
      cmStrCat(providerDir, "/provider.last");
    std::string const providerBefore = readFile(providerStateFile);
    std::string const globVerifyScript =
      cmStrCat(buildDir, "/CMakeFiles/VerifyGlobs.cmake");
    if (cmSystemTools::FileExists(globVerifyScript)) {
      std::vector<std::string> verifyCommand = {
        cmSystemTools::GetCMakeCommand(), "-P", globVerifyScript
      };
      int verifyRet = 0;
      bool verifyOk = cmSystemTools::RunSingleCommand(
        verifyCommand, nullptr, nullptr, &verifyRet, buildDir.c_str(),
        cmSystemTools::OUTPUT_PASSTHROUGH, cmDuration::zero(),
        cmProcessOutput::Auto);
      if (!verifyOk || verifyRet != 0) {
        return verifyRet == 0 ? 1 : verifyRet;
      }
    }
    std::vector<std::string> regenCommand = {
    ...
    int regenRet = 0;
    bool regenOk = cmSystemTools::RunSingleCommand(
      regenCommand, nullptr, nullptr, &regenRet, buildDir.c_str(),
      cmSystemTools::OUTPUT_PASSTHROUGH, cmDuration::zero(),
      cmProcessOutput::Auto);
    if (!regenOk || regenRet != 0) {
      return regenRet == 0 ? 1 : regenRet;
    }
    std::string const providerAfter = readFile(providerFile);
    if (!providerBefore.empty() && providerBefore != providerAfter) {
      std::vector<std::string> worktrees;
      cmSystemTools::GlobDirs(cmStrCat(providerDir, "/worktrees/*"),
                              worktrees);
      for (std::string const& worktree : worktrees) {
        cmSystemTools::RemoveFile(
          cmStrCat(worktree,
                   "/build/reprobuild/provider-graph/provider-fragments.rbsz"));
      }
      std::string const cleanManifest = metadataValue("clean_manifest");
      std::ifstream cleanIn(cleanManifest.c_str());
      std::string cleanPath;
      while (std::getline(cleanIn, cleanPath)) {
        if (!cleanPath.empty() && cmSystemTools::FileExists(cleanPath)) {
          cmSystemTools::RemoveFile(cleanPath);
        }
      }
    }
    if (!providerAfter.empty()) {
      std::ofstream stateOut(providerStateFile.c_str(),
                             std::ios::out | std::ios::binary);
      stateOut << providerAfter;
    }
  }
